...
class Renderer {
    ...
    */
    ...
    render() {
        ...
        var light_pos = this.scene.light.newLightPosition();
        ...
        for (var i = 0; i < this.scene.geometries.length; i++) {
          var geometry = this.scene.geometries[i];
            ...
            geometry.shader.changeUniform("u_EyePosition", this.camera.eye.elements);
            geometry.shader.changeUniform("u_ViewMatrix", this.camera.viewMatrix.elements);
            geometry.shader.changeUniform("u_ProjectionMatrix", this.camera.projectionMatrix.elements);
            ...
            if(this.scene.light != null) {
              geometry.shader.changeUniform("u_LightPos", light_pos.elements);
              geometry.shader.changeUniform("u_AmbientColor", this.scene.light.ambient);
              geometry.shader.changeUniform("u_DiffuseColor", this.scene.light.diffuse);
              geometry.shader.changeUniform("u_SpecularColor", this.scene.light.specular);
            }
            ...
            if(geometry instanceof Circle){
              this.drawBuffer(geometry.indices.length, true);
            } else {
              this.drawBuffer(geometry.indices.length, false);
            }
          }
    }
