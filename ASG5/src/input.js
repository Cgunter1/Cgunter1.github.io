...
class InputHandler {
    ...
    constructor(canvas, scene, doc, camera) {
      ...
      _inputHandler.scene.addGeometry(new Sphere(normalShader, 13, [.5, 0, 0], -10, 0));
      _inputHandler.scene.addGeometry(new Sphere(normalShader, 13, [.5, 0, 0], 12, -2));
      this.ground_image.onload = function() {
          // _inputHandler.sky_image.onload = function(){
        _inputHandler.scene.addGeometry(new Square(normalShader, 36, 0, 0, [0, .2 , 0], true, null));
        // _inputHandler.scene.addGeometry(new Triangle(normalShader, .75, .25, 0, [1, 0, 0], true, null));
        _inputHandler.scene.addGeometry(new TexCubeWithZ(normalShader, 1, -12, .3, -16, [0, 0, 0], false, _inputHandler.ground_image));
      ...
      this.wall_image.onload = function(){
        for(let i = 0; i < _inputHandler.mapLayout.length; ++i){
          ...
          _inputHandler.scene.addGeometry(new TexCubeWithZ(normalShader, 1, x, 0, z, [1, 0, 0], true, _inputHandler.wall_image));
          for(let j = 1; j < _inputHandler.mapLayout[i][0]; ++j){
            _inputHandler.scene.addGeometry(new TexCubeWithZ(normalShader, 1, x, j, z, [1, 0, 0], true, _inputHandler.wall_image));
